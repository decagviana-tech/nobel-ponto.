...
import { Save, Printer, Filter, CalendarDays, Zap, Unlock, KeyRound, Edit } from 'lucide-react';
...
export const SpreadsheetView: React.FC<Props> = ({ onUpdate, employeeId }) => {
  ...
  const fullMonthRecords = daysInMonth.map(dateStr => {
      ...
      return { date: dateStr, employeeId, entry: '', lunchStart: '', lunchEnd: '', snackStart: '', snackEnd: '', exit: '', totalMinutes: 0, balanceMinutes: isPast ? -target : 0 };
  });
  ...
  return (
    <div className="w-full overflow-hidden bg-white rounded-[2.5rem] shadow-sm border border-slate-200 flex flex-col h-full print:shadow-none">
      ...
      <div id="printable-content" className="overflow-x-auto">
        ...
        <thead className="text-[9px] text-slate-400 uppercase font-black bg-slate-50 sticky top-0">
            <tr>
            ...
            <th className={`px-6 py-5 text-center print:hidden ${!isUnlocked ? 'opacity-0' : ''}`}>Ação</th>
            </tr>
        </thead>
        <tbody className="divide-y divide-slate-100">
            ...
            return (
                ...
                <td className="px-6 py-4 text-center print:hidden">{isEditing ? <button onClick={handleSave} className="text-white bg-emerald-600 rounded-xl p-2 shadow-lg"><Save size={14} /></button> : <button onClick={() => handleEditClick(record as any)} className={`p-2 transition-all ${isUnlocked ? 'text-brand-600 scale-110' : 'opacity-0 pointer-events-none'}`}><Edit size={14} /></button>}</td>
                </tr>
            );
            })}
        </tbody>
