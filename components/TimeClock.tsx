...
export const TimeClock: React.FC<Props> = ({ onUpdate, employeeId }) => {
  ...
  const ActionButton = ({ type, label, icon: Icon, colorClass }: any) => {
    ...
    return (
      <button
        ...
        className={`
          relative flex flex-col items-center justify-center p-4 md:p-8 rounded-[2rem] transition-all duration-300 shadow-lg border-2
          ${isDone 
            ? 'bg-slate-50 border-slate-100 text-slate-300 scale-95 opacity-80' 
            : isRecommended
              ? `${colorClass} text-white border-transparent scale-100 ring-[8px] ring-brand-100/50 shadow-brand-200/50 z-10 font-black`
              : 'bg-white border-slate-100 text-slate-600 hover:border-brand-200 active:scale-95'
          }
        `}
      >
        {isRecommended && !isDone && (
            <div className="absolute -top-3 -right-3 bg-brand-500 text-white p-2 rounded-full shadow-lg animate-bounce border-2 border-white">
                <Zap size={16} fill="white" />
            </div>
        )}
        {isLoading ? <Loader2 size={24} className="animate-spin mb-3" /> : <Icon size={24} className="mb-3" />}
        <span className="text-sm tracking-tight uppercase font-black">{isDone ? todayRecord[type as keyof DailyRecord] : label}</span>
        {isDone && <span className="mt-1 text-[9px] font-black tracking-widest opacity-60 uppercase">OK</span>}
      </button>
    );
  };
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-4xl mx-auto p-4 animate-fade-in">
      {successMsg && (
          <div className="fixed top-20 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-6 py-4 rounded-full shadow-2xl flex items-center gap-4 z-[100] animate-in slide-in-from-top-10 duration-300 border border-slate-700">
              <div className="bg-emerald-500 p-1 rounded-full"><CheckCircle2 size={18} /></div>
      ...
      <div className="text-center mb-6 md:mb-8 w-full">
        ...
        <div className="relative inline-block">
            <h2 className="text-5xl md:text-7xl leading-none font-black text-slate-900 font-mono tracking-tighter transition-all duration-300">
                {currentTime}
            </h2>
            <div className="mt-3 h-1 bg-brand-100 rounded-full overflow-hidden w-full">
                <div className="h-full bg-brand-500 animate-progress w-full"></div>
            </div>
        </div>
        
        <div className="mt-6">
            <div className={`inline-flex items-center gap-2 px-6 py-2 rounded-full text-xs font-black border transition-all
                ${totalBankBalance >= 0 ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'}
            `}>
                <Wallet size={16} />
                SALDO: {formatTime(totalBankBalance)}
            </div>
        </div>
      </div>
      ...
      <style>{`
        ...
        .animate-progress { animation: progress 60s linear infinite; }
      `}</style>
    </div>
  );
};
