...
import { Utensils, LogOut, LogIn, MapPin, Wallet, Loader2, CheckCircle2, ShieldCheck, Zap, Info } from 'lucide-react';
...
export const TimeClock: React.FC<Props> = ({ onUpdate, employeeId }) => {
  ...
  const getLabel = (type: string) => {
    const labels: any = { entry: 'Entrada', lunchStart: 'Almoço (Ida)', lunchEnd: 'Almoço (Volta)', exit: 'Saída' };
  ...
  const ActionButton = ({ type, label, icon: Icon, colorClass }: any) => {
    ...
    return (
      <button
        ...
        className={`relative flex flex-col items-center justify-center p-4 md:p-8 rounded-[2rem] transition-all duration-300 shadow-lg border-2 ${isDone ? 'bg-slate-50 border-slate-100 text-slate-300 scale-95 opacity-80' : isRecommended ? `${colorClass} text-white border-transparent ring-[8px] ring-brand-100/50 shadow-brand-200/50 z-10 font-black` : 'bg-white border-slate-100 text-slate-600 hover:border-brand-200 active:scale-95'}`}
      >
        {isRecommended && !isDone && <div className="absolute -top-3 -right-3 bg-brand-500 text-white p-2 rounded-full shadow-lg animate-bounce border-2 border-white"><Zap size={16} fill="white" /></div>}
